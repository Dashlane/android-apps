...
import com.dashlane.csvimport.matchcsvfields.MatchCsvFieldsActivity.Companion.CATEGORY_NONE
...
internal class MatchCsvFieldsViewProxy(
    private val activity: AppCompatActivity,
    private val viewModel: MatchCsvFieldsViewModel
) {
    private val rvAdapter = DashlaneRecyclerAdapter<MatchCsvFieldsItem>().apply {
        onItemClickListener = EfficientAdapter.OnItemClickListener { _, _, _, _ ->
            toggleHeader()
        }
    }
    ...
    private val pageIndicator = activity.findViewById<TabLayout>(R.id.page_indicator)!!
        ...
        activity.lifecycleScope.launch {
            activity.repeatOnLifecycle(Lifecycle.State.STARTED) {
                viewModel.stateFlow.collect { (items, position, canValidate) ->
                    ...
                    pageIndicator.clearOnTabSelectedListeners()
                    items.forEachIndexed { index, _ ->
                        pageIndicator.addTab(pageIndicator.newTab(), position == index)
                    }
                    pageIndicator.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
                        override fun onTabSelected(tab: TabLayout.Tab) {
                            viewModel.goToPosition(tab.position)
                        }
    ...
    private fun updateView(items: List<MatchCsvFieldsItem>, position: Int, canValidate: Boolean) {
