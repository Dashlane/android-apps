...
class RankingSearchSorter @Inject constructor(
    @ApplicationContext context: Context,
    fieldMatcher: SearchImprovementsUtils
) : SearchSorter {
    ...
    private fun findMatch(
        item: Any,
        query: String,
        filter: Map<Any, List<(Any, String) -> Match?>>
    ): MatchedSearchResult? {
        val match = when (item) {
            is SummaryObject -> {
    ...
    private fun SyncObjectType.toSearchItemType(): ItemType = when (this) {
