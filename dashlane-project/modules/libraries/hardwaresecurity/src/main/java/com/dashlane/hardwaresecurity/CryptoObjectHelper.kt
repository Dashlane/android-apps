...
class CryptoObjectHelper @Inject constructor(
    ...
    fun challengeAuthentication(cipher: Cipher): CryptoChallengeResult {
        return try {
            ...
            CryptoChallengeResult.Failure(
                throwable = e,
                errorMessage = "Cryptographic Key is not properly stored, Biometric Authentication can't be securely used"
            )
        }
    }
    ...
    private fun getCipher(
        ...
        if (keyStoreKey is BiometricsSeal && !preferencesManager[keyStoreKey.username].biometricSealPaddingMigrationAttempt) {
    ...
    private fun migrateEncryptionKey(keyStoreKey: BiometricsSeal) {
        
        
        
        deleteEncryptionKey(keyStoreKey)
        createEncryptionKey(keyStoreKey, true)
        preferencesManager[keyStoreKey.username].biometricSealPaddingMigrationAttempt = true
    }
    ...
    sealed class CryptoChallengeResult {
        data object Success : CryptoChallengeResult()
