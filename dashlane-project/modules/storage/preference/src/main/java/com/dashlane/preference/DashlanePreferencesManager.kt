...
abstract class DashlanePreferencesManager {
    ...
    fun apply(preferenceEntries: PreferenceEntry): Boolean {
        return apply(listOf(preferenceEntries))
    }
    ...
    fun remove(labels: List<String>): Boolean {
        return apply(labels.map { PreferenceEntry.toRemove(it) })
    }
