...
class CreateAccountViewModel @Inject constructor(
    ...
    private val teamspaceAccessor: TeamspaceAccessor? by lazy {
    ...
    fun save(userInputWebsite: String?, login: String?, password: String?, spaceSelectedIndex: Int) {
        viewModelScope.launch {
            ...
            val result = withContext(ioCoroutineDispatcher) {
                ...
                val spaceId = stateFlow.value.data.teamSpace?.getOrNull(spaceSelectedIndex)?.teamId
                val info = CredentialInfo(
                    provider.getCredentialTitle(context, website, packageName, userInputWebsite),
                    userInputWebsite,
                    login!!,
                    password!!,
                    packageName,
