        ...
        data class BaseOffer(
            ...
            override fun getCtaString(context: Context): String {
                val stringRes = when (baseOfferPeriodicity.periodicity) {
                    ProductPeriodicity.MONTHLY -> R.string.plan_details_price_billed_monthly
                    ProductPeriodicity.YEARLY -> R.string.plan_details_price_billed_yearly
                }
            ...
            override fun getCtaString(context: Context): String {
                ...
                return if (offerTotalLength == 1) {
                    val textResource = when (periodicity) {
                        ProductPeriodicity.MONTHLY -> R.plurals.plan_details_pending_offer_recurring_payment_price_monthly
                        ProductPeriodicity.YEARLY -> R.plurals.plan_details_pending_offer_recurring_payment_price_yearly
                    }
                    ...
                    val textResource = when (periodicity) {
                        ProductPeriodicity.MONTHLY -> R.plurals.plan_details_pending_offer_single_payment_price_monthly
                        ProductPeriodicity.YEARLY -> R.plurals.plan_details_pending_offer_single_payment_price_yearly
                    }
                    TextResource.PluralsText(
                        pluralsRes = textResource,
                        quantity = offerTotalLength,
