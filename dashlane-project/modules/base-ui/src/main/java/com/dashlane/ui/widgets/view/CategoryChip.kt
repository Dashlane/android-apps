...
@Composable
fun CategoryChip(label: String, modifier: Modifier = Modifier, editMode: Boolean = false, onClick: () -> Unit = {}) {
    DashlaneTheme {
        InputChip(
            onClick = onClick,
            selected = false,
            label = {
                Text(label, color = DashlaneTheme.colors.textNeutralStandard, style = DashlaneTheme.typography.bodyReducedRegular)
            },
