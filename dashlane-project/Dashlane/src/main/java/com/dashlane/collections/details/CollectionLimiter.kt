...
class CollectionLimiter @Inject constructor(
    ...
    private val hasCollectionSharing: Boolean
        get() = userFeatureChecker.has(PremiumStatus.Capabilitie.Capability.COLLECTIONSHARING)

    private val info: PremiumStatus.Capabilitie.Info?
        get() = userFeatureChecker.getCapabilityInfo(
            PremiumStatus.Capabilitie.Capability.COLLECTIONSHARING
        )

    private val isCurrentTeamTrial: Boolean
        get() = accountStatusRepository[session]?.premiumStatus?.isCurrentTeamTrial == true

    private val reason: PremiumStatus.Capabilitie.Info.Reason?
        get() = info?.reason

    private val limit: Long
        get() = info?.limit ?: 0

    private val whoCanShare: PremiumStatus.Capabilitie.Info.WhoCanShare?
    ...
    suspend fun checkCollectionLimit(isShared: Boolean = false): UserLimit {
        if (!hasCollectionSharing) {
            return if (reason == PremiumStatus.Capabilitie.Info.Reason.NOT_ADMIN) {
                UserLimit.NOT_ADMIN
            } else {
                UserLimit.DISABLED
            }
        ...
        return when {
            ...
            whoCanShare == PremiumStatus.Capabilitie.Info.WhoCanShare.EVERYONE -> UserLimit.NO_LIMIT
            else -> {
