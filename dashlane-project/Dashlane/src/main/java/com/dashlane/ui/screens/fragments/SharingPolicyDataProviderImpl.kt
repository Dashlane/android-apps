...
class SharingPolicyDataProviderImpl @Inject constructor(
    ...
    override fun canShareItem(summaryObject: SummaryObject): Boolean {
    ...
    override fun getSharingCount(uid: String): Pair<Int, Int> {
    ...
    override fun getSharingCountUserAndUserGroup(itemGroup: ItemGroup): Pair<Int, Int> {
    ...
    override fun isDeleteAllowed(uid: String, isNewItem: Boolean, isShared: Boolean): Boolean {
        if (isNewItem) {
            return false
        }
        if (!isShared) {
    ...
    override fun isDeleteAllowed(isNewItem: Boolean, item: VaultItem<SyncObject>?): Boolean {
        item ?: return false
        return isDeleteAllowed(item.uid, isNewItem, item.isShared())
    }

    override suspend fun doCancelSharingFor(
